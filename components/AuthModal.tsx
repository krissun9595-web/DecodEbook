...
function isInAppBrowser(): boolean {
  ...
  return /FBAN|FBAV|Instagram|Twitter|Line\/|wv|WebView|; wv\)/i.test(ua)
    || ((/iPhone|iPad|iPod/.test(ua) && !/Safari/.test(ua)))
    || (/Android/.test(ua) && /Version\//.test(ua) && !/Chrome/.test(ua));
}
        ...
        {user ? (
          <div className="space-y-4">
            <div className="p-3 bg-zinc-900 rounded border border-zinc-800">
              ...
              <p className="text-sm text-neon-cyan font-mono">{user.email}</p>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 bg-void-1 border border-zinc-800 rounded px-3 py-2">
            ...
            <button onClick={handleForgotPassword} disabled={loading} className="w-full py-2.5 bg-neon-cyan text-black font-bold rounded text-xs font-mono uppercase tracking-widest hover:bg-[#00c2cc] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {loading && <Loader2 size={14} className="animate-spin" />}
              Send Reset Link
            </button>
            <button onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="w-full py-2 text-zinc-500 hover:text-neon-cyan text-[10px] font-mono uppercase tracking-widest transition-colors">
              Back to Sign In
            </button>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-center">
              <button onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setError(''); setSuccess(''); setAgreedToTerms(false); }} className="text-[10px] text-zinc-500 hover:text-neon-cyan font-mono uppercase tracking-widest transition-colors">
            ...
            {mode === 'login' && (
              <p className="text-[9px] text-zinc-500 font-mono text-center leading-relaxed">
                ...
                {' '}and{' '}
                <a href="/privacy" target="_blank" className="text-zinc-500 hover:text-neon-cyan underline">Privacy Policy</a>
              </p>
            )}
          </div>
        )}
      ...
      <div className="w-full max-w-sm p-8 relative z-10">
        ...
        {inAppBrowser && (
          <div className="mb-4 p-3 bg-amber-950/20 border border-amber-900/40 rounded-lg space-y-2">
            <p className="text-[11px] text-amber-400 font-mono leading-relaxed text-center">
              Open in your browser for the best experience
            </p>
            <p className="text-[9px] text-zinc-500 font-mono text-center leading-relaxed">
              Tap <span className="text-zinc-300">⋯</span> or <span className="text-zinc-300">Safari / Chrome</span> icon above to open in browser
            </p>
            <button
              onClick={onSkip}
              className="w-full py-2 bg-neon-cyan text-black font-bold rounded text-[10px] font-mono uppercase tracking-widest hover:bg-[#00c2cc] transition-all flex items-center justify-center gap-1.5"
            >
        ...
        {mode === 'forgot' ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 bg-void-1 border border-zinc-800 rounded px-3 py-2.5">
              <Mail size={14} className="text-zinc-600 shrink-0" />
          ...
          <div className="space-y-4">
            ...
            {isSupabaseConfigured() && (
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => handleOAuth('google')} disabled={loading || inAppBrowser} className={`py-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 border border-zinc-800 rounded text-xs font-mono uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-50 ${inAppBrowser ? 'opacity-30 cursor-not-allowed' : ''}`} title={inAppBrowser ? 'Open in browser to use Google sign-in' : ''}>
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> Google
                  </button>
                  <button onClick={() => handleOAuth('github')} disabled={loading} className="py-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 border border-zinc-800 rounded text-xs font-mono uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                    <Github size={14} /> GitHub
