...
export const SettingsModal: React.FC<Props> = ({ isOpen, onClose, settings, onUpdate }) => {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Settings" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in font-sans">
      <div className="bg-void-1 border border-zinc-800 rounded-lg w-full max-w-md shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-fade-in-up scale-in relative">
        ...
        <div className="p-4 bg-zinc-900 border-t border-zinc-800 flex justify-end">
           <button 
             onClick={onClose}
             className="px-8 py-2.5 bg-neon-cyan hover:bg-[#00c2cc] text-black rounded-sm font-bold uppercase tracking-wider transition-all shadow-[0_0_15px_rgba(0,243,255,0.3)] hover:shadow-[0_0_25px_rgba(0,243,255,0.5)] font-mono text-xs"
           >
             Apply_Changes
           </button>
        </div>
