...
const CHAPTER_TEXT_CACHE_VERSION = 'v238-right-list-split';
...
interface ParagraphData {
    ...
    smallCaps?: boolean;
    // Chapter-opener DROP CAP (U+E02E) — the source floats an oversized ::first-letter. Reproduced by a
    // `::first-letter` CSS class on the paragraph's first line (an inline style can't target the pseudo).
    dropCap?: boolean;
    // A heading/paragraph the source COLORS (U+E030-E035) — the reader-palette accent nearest the source
    // color by hue (Transurfing's teal headings → cyan). Kept as a hex the render applies as `color`.
    accentColor?: string;
          ...
          <div className="flex-1 overflow-hidden rounded-sm border border-zinc-800 bg-void-1 relative flex flex-col hud-border text-left">
             <div ref={readerScrollRef} data-reader-zone="" className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-6 space-y-0 pb-32 content-font">
                ...
                {paragraphData.map((para, pIdx) => {
                  ...
                  if (para.calloutType) {
                    // An admonition callout: a labelled bordered box. Warning gets a red-tinted border+label
                    // (reader neon-red), note/tip neutral. In SPLIT view render TWO boxes side by side
                    // (original left, translation right) — like code/table — so the middle orig/trans divider
